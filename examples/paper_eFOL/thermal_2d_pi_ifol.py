...
def main(ifol_num_epochs=10,solve_FE=False,clean_dir=False):
    # cleaning & logging
    working_directory_name = 'nn_output_thermal_pi_ifol'
    ...
    ifol_settings_dict = {
        ...
        "main_loop_transform": optax.linear_schedule(init_value=1e-5, end_value=1e-7, transition_steps=ifol_num_epochs),
        "latent_step_optimizer": optax.linear_schedule(init_value=1e-4, end_value=1e-6, transition_steps=ifol_num_epochs),
    ...
    # learning_rate_scheduler = optax.linear_schedule(init_value=1e-5, end_value=1e-7, transition_steps=ifol_num_epochs)
