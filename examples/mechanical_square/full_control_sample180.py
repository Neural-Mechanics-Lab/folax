...
import matplotlib.pyplot as plt                            # <-- for Newton plots

def main(fol_num_epochs=10, solve_FE=False, clean_dir=False, restore_param_init=True):
    # ============================================================
    # 0) directory & save handling
    #    (separate folder from slide 2!)
    # ============================================================
    working_directory_name = 'TransferLearnOTF_sample_180'
    ...
    # fourier control
    fourier_control_settings = {"x_freqs": np.array([2, 4, 6]),
    ...
    # ============================================================
    # 2) load parametric samples (same as slide 2)
    # ============================================================
    create_random_coefficients = False
    if create_random_coefficients:
        number_of_random_samples = 200
        coeffs_matrix, K_matrix = create_random_fourier_samples(
            fourier_control, number_of_random_samples
        )
    ...
    # ============================================================
    # 3) FNO model definition (same as slide 2)
    # ============================================================
    fno_model = bridge.ToNNX(
        ...
        K_matrix[0:1].reshape(1, model_settings["N"], model_settings["N"], 1)
    )
    ...
    # ============================================================
    # 5) restore parametric weights from slide 2 (optional)
    # ============================================================
    param_case_dir  = './PARAMETRIC_sample_180'
    ...
    if restore_param_init:
        ...
        print("[INFO] Skipping parametric checkpoint restore. Starting from current initialization.")

    # ============================================================
    # 6) OTF setup: fine-tune only on sample 180
    # ============================================================
    otf_id = 180
    train_start_id = 0
    train_end_id = 180
    ...
    parametric_learning = False   # <-- now on-the-fly only
    ...
    # ============================================================
    # 7) OTF training (initialized from parametric weights or random)
    # ============================================================
    pi_fno_pr_learning.Train(
        ...
        # residual_rms_batch_mean tracker included here
        plot_settings={
    ...
    # after OTF training, restore best fine-tuned state from THIS run
    pi_fno_pr_learning.RestoreState(
        restore_state_directory=os.path.join(case_dir, "flax_train_state")
    )

    # ============================================================
    # 8) Evaluation on the same OTF sample (180)
    # ============================================================
    for eval_id in eval_cases:
        # ---------- FNO prediction ----------
        FNO_UV = np.array(
            pi_fno_pr_learning.Predict(
                coeffs_matrix[eval_id, :].reshape(-1, 1).T
            )
        ).reshape(-1)
        fe_mesh[f'U_FNO_{eval_id}'] = FNO_UV.reshape(
        ...
        # baseline Newton: start from zero DOFs
        ndofs = 2 * fe_mesh.GetNumberOfNodes()
        initial_dofs = np.zeros(ndofs)

        # custom Newton solve with residual tracking
        ...
        fe_mesh[f'U_FE_{eval_id}'] = FE_UV.reshape(
            (fe_mesh.GetNumberOfNodes(), 2)
        )
        ...
        plt.title(f"Newton convergence – sample {eval_id}")
        plt.grid(True, which="both", ls="--", alpha=0.5)
        plt.tight_layout()
        plt.savefig(os.path.join(
            case_dir, f"newton_residuals_sample_{eval_id}.png"
        ))
        plt.close()
        ...
        plot_mesh_vec_data(
            ...
            file_name=os.path.join(
                case_dir, f'plot_results_{eval_id}.png'
            )
        )
...
if __name__ == "__main__":
    ...
    for arg in args:
        if arg.startswith("fol_num_epochs="):
            ...
            print("Usage: python mechanical_2D.py "
                  "fol_num_epochs=10 solve_FE=False clean_dir=False restore_param_init=True")
            sys.exit(1)

    # Call the main function with the parsed values
    main(fol_num_epochs, solve_FE, clean_dir, restore_param_init)
