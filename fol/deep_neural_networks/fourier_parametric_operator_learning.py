...
class FourierParametricOperatorLearning3D(DeepNetwork):
    ...
    def ComputeBatchPredictions(self,batch_X:jnp.ndarray,nn_model:nnx.Module):
        batch_size = batch_X.shape[0]
        mesh_size = int(np.cbrt(batch_X.shape[1]))
        num_chs = int(batch_X.size/(mesh_size*mesh_size*mesh_size*batch_size))
        return nn_model(batch_X.reshape(batch_size,mesh_size,mesh_size,mesh_size,num_chs)).reshape(batch_size,-1)
